...
void CIt_process_list_compressed_qmx_d4(uint8_t *source, uint8_t *end, uint16_t impact,
	...
	qmx_d4_decoder.decodeArray((uint32_t *)source, end - source, CI_decompressed_postings,
	                           integers);
...
void CIt_process_list_compressed_simple8b(uint8_t *source, uint8_t *end, uint16_t impact,
	...
	while (compressed_sequence < (uint64_t *)end) {
		...
		case 0xD:
			sum += value & 0xFFFFF;
			add_rsv(sum, impact);
			sum += (value >> 20) & 0xFFFFF;
			add_rsv(sum, impact);
			sum += (value >> 40) & 0xFFFFF;
			add_rsv(sum, impact);
			break;
		case 0xE:
			sum += value & 0x3FFFFFFF;
			add_rsv(sum, impact);
			sum += (value >> 30) & 0x3FFFFFFF;
			add_rsv(sum, impact);
			break;
		case 0xF:
			sum += value & 0xFFFFFFFFFFFFFFFL;
			add_rsv(sum, impact);
			break;
		}
	}

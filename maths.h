...
/*
    ...
    -----------------
*/
static inline double ANT_log_to_base(double base, double value) { return log(value) / log(base); }
...
*/
template <class Type> Type ANT_max(Type first, Type second) {
	return first > second ? first : second;
}
template <class Type> Type ANT_max(Type first, Type second, Type third) {
	return ANT_max(ANT_max(first, second), third);
}
...
*/
template <class Type> Type ANT_min(Type first, Type second) {
	return first < second ? first : second;
}
template <class Type> Type ANT_min(Type first, Type second, Type third) {
	return ANT_min(ANT_min(first, second), third);
}
...
/*
    ...
    ------------
*/
static inline double ANT_secant(double x1, double x2, double (*function)(double, void *parameter),
                                void *function_param) {
...
*/
static inline double ANT_falsi_method(double s, double t, double e, int m,
                                      double (*function)(double, void *parameter),
                                      void *function_param) {
	double r = 0, fr;
	int n, side = 0;
...
*/
static inline double ANT_bisection_method(double x0, double x1,
                                          double (*function)(double, void *parameter),
...
*/
static inline double ANT_gradient_descent(double guess, double eps, double precision, double d_gap,
                                          double (*function)(double, void *parameter),
                                          void *function_parameter) {
	double f_prime;
	double x_old, x_new;
	...
	do {
		x_old = x_new;
		f_prime =
		    (function(x_old + d_gap, function_parameter) - function(x_old, function_parameter)) /
		    d_gap;
...
/*
    ...
    ---------------------------------
    Code for computing logs of arbitrary bases at compile-time. This allows logs to be computed as
   part of constant expressions.
*/
template <unsigned long long n, unsigned int base> struct ANT_compiletime_floor_log_to_base {
	enum { value = n < base ? 0 : 1 + ANT_compiletime_floor_log_to_base<n / base, base>::value };
};

template <unsigned int base> struct ANT_compiletime_floor_log_to_base<0, base> {
	enum { value = 0 };
};
...
/*
    ...
    --------------------------
*/
template <int n> struct ANT_compiletime_ispowerof2 {
	enum { value = !(n & (n - 1)) };
};
...
/*
    ...
    --------------------------
*/
template <unsigned long long n> struct ANT_compiletime_floor_log2 {
	enum { value = 1 + ANT_compiletime_floor_log2<(n >> 1)>::value };
};

template <> struct ANT_compiletime_floor_log2<0> {
	enum { value = 0 };
};

template <> struct ANT_compiletime_floor_log2<1> {
	enum { value = 0 };
};
...
/*
    ...
    -------------------
*/
template <unsigned int base, unsigned int exponent> struct ANT_compiletime_pow {
	static const unsigned long long value = base * ANT_compiletime_pow<base, exponent - 1>::value;
};

template <unsigned int base> struct ANT_compiletime_pow<base, 1> {
	static const unsigned long long value = base;
};

template <unsigned int base> struct ANT_compiletime_pow<base, 0> {
...
*/
template <typename T> struct ANT_compiletime_int_max {
	static const T value =
	    (T)(std::numeric_limits<T>::is_signed ? ~(T)(1ULL << (sizeof(T) * CHAR_BIT - 1)) : ~0ULL);
};

/*
    ANT_COMPILETIME_INT_FLOOR_LOG_TO_BASE
    -------------------------------------
    How many 'base' digits would fit into an integer of type T? This is required in addition to
   ANT_compiletime_floor_log_to_base,
    because we need to be able to compute it precisely for the largest supported integral type, too.
*/
template <typename T, int base> struct ANT_compiletime_int_floor_log_to_base {
	/*
	    Avoid creating the value 1 << sizeof(T) * CHAR_BIT, which can't fit in T. The only case
	   where this is important
	    is when 'base' is a power of two (and so could fit an integer number of times into T),
	   otherwise we can take the
	    log of 1 << sizeof(T) * CHAR_BIT - 1 instead.
	*/
	enum {
		value = (ANT_compiletime_ispowerof2<base>::value
		             ? (sizeof(T) * CHAR_BIT - (std::numeric_limits<T>::is_signed ? 1 : 0)) /
		                   ANT_compiletime_floor_log2<base>::value
		             : ANT_compiletime_floor_log_to_base<ANT_compiletime_int_max<T>::value,
		                                                 base>::value)
	};
};
...
/*
    ...
    ---------------------------------------------------
    Is there a remainder when computing the log of the integer type T (i.e. 2^(num_bits_in_T)) to
   the given base?
*/
template <typename T, int base> struct ANT_compiletime_int_floor_log_to_base_has_remainder {
	/*
	    The only way to avoid a remainder is to have base be a power of two where the number of bits
	   required to represent it fits exactly into T
	*/
	enum {
		value = (ANT_compiletime_ispowerof2<base>::value
		             ? ((sizeof(T) * CHAR_BIT - (std::numeric_limits<T>::is_signed ? 1 : 0)) %
		                ANT_compiletime_floor_log2<base>::value) != 0
		             : 1)
	};
};
...
struct ANT_compiletime_int_floor_log_to_base_remainder {
	enum {
		value =
		    ANT_compiletime_int_max<T>::value /
		    ANT_compiletime_pow<base, ANT_compiletime_int_floor_log_to_base<T, base>::value>::value
	};
};

template <typename T, int base> struct ANT_compiletime_int_floor_log_to_base_remainder<T, base, 0> {
	enum { value = 1 };
};

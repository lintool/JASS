...
/*
    ...
    --------------
*/
template <typename T, typename _Compare = ANT_primary_cmp<T>> class ANT_heap {
  private:
	T *array;
	long long size;
  ...
  public:
	ANT_heap(T &arry, long long size, const _Compare &__c = _Compare())
	    : array(&arry), size(size), compare(__c){};
...
/*
    ...
    ----------------------
*/
template <typename T, typename _Compare> int ANT_heap<T, _Compare>::min_insert(T key) {
	long long i = 0, lpos, rpos;
...
void ANT_heap<T, _Compare>::max_heapify(long long pos, long long hsize) {
	long long left = left_pos(pos);
	long long right = right_pos(pos);
	long long largest;
...
*/
template <typename T, typename _Compare>
void ANT_heap<T, _Compare>::min_heapify(long long pos, long long hsize) {
	long long left = left_pos(pos);
	long long right = right_pos(pos);
	long long smallest;

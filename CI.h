...
*/
struct add_rsv_compare {
	__forceinline int operator()(uint16_t *a, uint16_t *b) const {
		return *a > *b ? 1 : *a < *b ? -1 : (a > b ? 1 : a < b ? -1 : 0);
	}
};

extern uint16_t *CI_accumulators;          // the accumulators
extern uint16_t **CI_accumulator_pointers; // an array of pointers into the accumulators (used to
                                           // avoid computing docIDs)
...
extern uint32_t CI_accumulators_shift;  // number of bits to shift (right) the docid by to get the
...
class CI_vocab {
  public:
	const char *term;
	void **methods; // should be, but to make it compile faster it isn't : struct CI_impact_method
	                // **methods;
	uint64_t impacts;

  public:
	static int compare(const void *a, const void *b) {
		return strcmp(((CI_vocab *)a)->term, ((CI_vocab *)b)->term);
	}
	static int compare_string(const void *a, const void *b) {
		return strcmp((char *)a, ((CI_vocab *)b)->term);
	}
};
...
class CI_vocab_heap {
  ...
  public:
	static int compare(const void *a, const void *b) {
		return strcmp(((CI_vocab_heap *)a)->term, ((CI_vocab_heap *)b)->term);
	}
	static int compare_string(const void *a, const void *b) {
		return strcmp((char *)a, ((CI_vocab_heap *)b)->term);
	}
};
...
/*
    ADD_RSV()
    ---------
    This method cannot be forced inline because when I do so clang generates code that (sometimes)
   doesn't work!
*/
...
#endif
{
	...
	*/
	if (CI_accumulator_clean_flags[docid >> CI_accumulators_shift] == 0) {
		...
		memset(CI_accumulators + (CI_accumulators_width * (docid >> CI_accumulators_shift)), 0,
		       CI_accumulators_width * sizeof(uint16_t));
	}

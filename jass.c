...
uint16_t **CI_accumulator_pointers; // an array of pointers into the accumulators (used to avoid
                                    // computing docIDs)
...
uint32_t CI_accumulators_shift;      // number of bits to shift (right) the docid by to get the
                                     // CI_accumulator_clean_flags
...
void trec_dump_results(uint32_t topic_id, FILE *out, uint32_t output_length) {
	...
	for (current = 0; current < (output_length < CI_results_list_length ? output_length
	                                                                    : CI_results_list_length);
	     current++) {
		id = CI_accumulator_pointers[current] - CI_accumulators;
		fprintf(out, "%d Q0 %s %d %d COMPILED (ID:%u)\n", topic_id, CI_documentlist[id],
		        current + 1, CI_accumulators[id], id);
	}
}
...
char *read_entire_file(const char *filename, uint64_t *length = NULL) {
	...
	if (fstat(fileno(fp), &details) == 0) {
		if (details.st_size != 0) {
			if ((block = new char[(size_t)(details.st_size + 1)]) !=
			    NULL) { // +1 for the '\0' on the end
				if (length != NULL) {
...
int quantum_compare(const void *a, const void *b) {
	...
	/*
	 sort from highest to lowest impact, but break ties by placing the lowest quantum-frequency
	 first and the highest quantum-drequency last
	 */
	return lhs->impact > rhs->impact
	           ? -1
	           : lhs->impact < rhs->impact
	                 ? 1
	                 : lhs->quantum_frequency > rhs->quantum_frequency
	                       ? 1
...
void print_postings_list(CI_vocab_heap *postings_list) {
	...
	for (current = 0; current < postings_list->impacts; current++) {
		...
		printf("OFFSET:%llx Impact:%hx Offset:%llx End:%llx docids:%u\n", data[current],
		       header->impact, header->offset, header->end, header->quantum_frequency);
...
void read_doclist(void) {
	...
	offset_base =
	    (uint64_t *)(doclist + length - (total_docs * sizeof(uint64_t) + sizeof(uint64_t)));
...
void read_vocab(void) {
	...
	CI_unique_terms = total_terms =
	    length / (sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint64_t));
...
int main(int argc, char *argv[]) {
	chrono::time_point<chrono::steady_clock, chrono::nanoseconds> full_query_timer =
	    chrono::steady_clock::now();
	chrono::time_point<chrono::steady_clock, chrono::nanoseconds> start_timer, end_timer,
	    full_query_without_io_timer, b_enc_timer, e_enc_timer;
	chrono::nanoseconds encoding_time;
	...
	chrono::nanoseconds encoding_tmp;
	...
	uint64_t queries_encoding[MAX_QUERIES];
...
#ifdef ANYTIME
	if (argc < 1 || argc > 5) {
		exit(printf("Usage:%s <queryfile> [<top-k-number>] [<num-postings-to-proces>] "
		            "[-d<ecompress then process>]\n",
		            argv[0]));
	}
	...
	if (*postings == 's') {
		...
		exit(printf(
		    "This index appears to be invalid as it is neither compressed nor not compressed!\n"));
	}
	...
	accumulators_needed =
	    CI_accumulators_width * CI_accumulators_height; // guaranteed to be larger than the highest
	                                                    // accumulagtor that can be initialised
	CI_accumulator_clean_flags = new uint8_t[CI_accumulators_height];

	/*
	 Create a buffer to store the decompressed postings lists
	 */
	CI_decompressed_postings =
	    new uint32_t[CI_unique_documents +
	                 1024]; // we add because some decompressors are allowed to overflow this buffer
	...
	/*
	 For QaaT early termination we need K+1 elements in the heap so that we can check that nothing
	 else can get into the top-k.
	 */
	CI_top_k++;
	...
	while (experimental_repeat < times_to_repeat_experiment) {
		...
		encoding_time = chrono::nanoseconds(0);
#ifdef TIME_EACH_QUERY_PHASE
		...
		while (fgets(buffer, sizeof(buffer), fp) != NULL) {
			...
			encoding_tmp = chrono::nanoseconds(0);
...
// Initialize the accumulators
			...
			stats_accumulator_time +=
			    chrono::duration_cast<chrono::nanoseconds>(end_timer - start_timer);
#endif
...
#endif
				postings_list =
				    (CI_vocab_heap *)bsearch(term, CI_dictionary, CI_unique_terms,
				                             sizeof(*CI_dictionary), CI_vocab_heap::compare_string);
#ifdef TIME_EACH_QUERY_PHASE
				end_timer = chrono::steady_clock::now();
				stats_vocab_time +=
				    chrono::duration_cast<chrono::nanoseconds>(end_timer - start_timer);
...
// Initializee the SaaT structures
				...
				if (postings_list != NULL) {
					...
					memcpy(current_quantum, postings + postings_list->offset,
					       postings_list->impacts * sizeof(*quantum_order));

					// Compute the maximum possibe impact score (that is, assume one document has
					// the maximum impact for each term)
					max_remaining_impact +=
					    ((CI_quantum_header *)(postings + *current_quantum))->impact;
			...
			*current_quantum = 0;
			 ...
			 */
			qsort(quantum_order, current_quantum - quantum_order, sizeof(*quantum_order),
			      quantum_compare);

#ifdef TIME_EACH_QUERY_PHASE
			end_timer = chrono::steady_clock::now();
			stats_quantum_prep_time +=
			    chrono::duration_cast<chrono::nanoseconds>(end_timer - start_timer);
#endif
/*
...
#ifdef ELIAS_FANO
				/*
				 * "Cheat" for EliasFano, encoded the uncompressed postings
				 */
				b_enc_timer = chrono::steady_clock::now();
				vector<uint32_t> p{(uint32_t *)(postings + current_header->offset),
				                   (uint32_t *)(postings + current_header->offset) +
				                       current_header->quantum_frequency};
				EFList ef = EFEncoder::encode(p.begin(), p.end());
				e_enc_timer = chrono::steady_clock::now();
				encoding_tmp +=
				    chrono::duration_cast<chrono::nanoseconds>(e_enc_timer - b_enc_timer);
				CIt_process_list_compressed_elias_fano(EFReader(ef), current_header->impact,
				                                       current_header->quantum_frequency);
#else
				(*process_postings_list)(postings + current_header->offset,
				                         postings + current_header->end, current_header->impact,
				                         current_header->quantum_frequency);
#endif
#ifdef TIME_EACH_QUERY_PHASE
				end_timer = chrono::steady_clock::now();
...
// Check to see if it's posible for the remaining impacts to affect the order of the top-k.
				...
				start_timer = chrono::steady_clock::now();
#endif
				// Subtract the current impact score and then add the next impact score for the
				// current term.
				...
				if (CI_results_list_length > CI_top_k - 1) {
					...
					/*
					 We need to run through the top-(k+1) to see if its possible for any re-ordering
					 to occur
					 1. copy the accumulators;
					 2. sort the top k + 1
					 3. go through consequative rsvs checking to see if reordering is possible
					 (check rsv[k] - rsv[k + 1])
					 */
					memcpy(quantum_check_pointers, CI_accumulator_pointers,
					       CI_top_k * sizeof(*quantum_check_pointers));
					top_k_qsort(quantum_check_pointers, CI_top_k, CI_top_k);

					early_terminate = true;

					for (partial_rsv = quantum_check_pointers;
					     partial_rsv < quantum_check_pointers + CI_top_k - 1; partial_rsv++) {
						if (**partial_rsv - **(partial_rsv + 1) <
						    max_remaining_impact) { // We're sorted from largest to smallest so a[x]
							                        // - a[x+1] >= 0
							early_terminate = false;
							break;
						}
...
#ifdef TIME_EACH_QUERY_PHASE
				end_timer = chrono::steady_clock::now();
				stats_early_terminate_check_time +=
				    chrono::duration_cast<chrono::nanoseconds>(end_timer - start_timer);
#endif
				if (early_terminate) {
...
// Sort the accumulator pointers to put the highest RSV document at the top of the list.
			...
			/*
			 At this point we know the number of hits (CI_results_list_length) and they can be
			 decode out of the CI_accumulator_pointers array
			 where CI_accumulator_pointers[0] points into CI_accumulators[] and therefore
			 CI_accumulator_pointers[0] - CI_accumulators is the docid
			...
			queries_encoding[total_number_of_topics - 1] = encoding_tmp.count();
#endif

			// Dump TREC output
			trec_dump_results(query_id, out, CI_top_k - 1); // Subtract 1 from top_k because we
			                                                // added 1 for the early termination
			                                                // checks.
		}
	}
	...
	end_timer = chrono::steady_clock::now();
	stats_total_time_to_search +=
	    chrono::duration_cast<chrono::nanoseconds>(end_timer - full_query_timer);
	print_os_time();
	...
	printf("Encoding time              (per query) : %12llu ns\n",
	       encoding_time.count() / total_number_of_topics);
	printf("--------------------------------------------------------\n");
	...
	for (i = 0; i < total_number_of_topics; i++) {
		printf("query id,postings processed,ns,enc:%llu,%d,%llu,%llu\n", queries_id[i],
		       queries_num_postings[i], queries_latency[i], queries_encoding[i]);
	}
#endif

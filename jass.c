...
void dump_rho_med(uint32_t topic_id, uint32_t sorted_retrieved, uint32_t postings, uint64_t ns) {
  ...
  memcpy(tmp_CI_accumulator_pointers, CI_accumulator_pointers, sizeof(uint16_t *) * CI_results_list_length);
	...
	std::cout << topic_id << "," << sorted_retrieved << "," << ns << "," << med << "," << postings
	          << std::endl;
}
...
int main(int argc, char *argv[]) {
	...
	chrono::nanoseconds dumping_med;
	...
	if (argc < 1 || argc > 6) {
		exit(printf("Usage:%s <queryfile> [<top-k-number>] [<num-postings-to-proces>] "
		            "[-d<ecompress then process>] [gold_standard file]\n",
		            argv[0]));
	}
	...
	printf("Loading gold standard...");
	fflush(stdout);
	if ((gs = fopen(argv[(argc == 6 ? 5 : 4)], "rb")) == NULL) {
		exit(printf("Can't open gold standard file: %s\n", argv[(argc == 6 ? 5 : 4)]));
	}
	char d_str[1024];
	char F2[256];
	char gid[1024];
	...
	if (*postings == 's') {
		...
		exit(printf("This index appears to be invalid as it is neither compressed nor not "
		            "compressed!\n"));
	}
	...
	CI_decompressed_postings =
	    new uint32_t[CI_unique_documents + 1024]; // we add because some decompressors are
	                                              // allowed to overflow this buffer
  ...
  tmp_CI_accumulator_pointers = new uint16_t *[accumulators_needed];
	...
	while (experimental_repeat < times_to_repeat_experiment) {
		...
		while (fgets(buffer, sizeof(buffer), fp) != NULL) {
			...
			dumping_med = chrono::nanoseconds(0);
...
// Initializee the SaaT structures
				...
				if (postings_list != NULL) {
					...
					// Compute the maximum possibe impact score (that is, assume one document
					// has the maximum impact for each term)
...
// Sort the accumulator pointers to put the highest RSV document at the top of the list.
	...
	for (i = 0; i < total_number_of_topics; i++) {
		printf("query id,postings processed,ns:%llu,%d,%llu\n", queries_id[i],
		       queries_num_postings[i], queries_latency[i]);
	}
#endif

/*
   ...
   the future be
    ...
    This version of QMX computes D4 deltas on compression and will un-delta on decompression so pass
   DOC-IDs not deltas.
    For details on D4, see: Daniel Lemire, Leonid Boytsov, Nathan Kurz, SIMD Compression and the
   Intersection of Sorted Integers, arXiv: 1401.6399, 2014 http://arxiv.org/abs/1401.6399
...
/*
    ...
    ----------------------------------
*/
void ANT_compress_qmx_d4::encodeArray(const uint32_t *raw_integers, uint64_t source_integers,
                                      uint32_t *into, uint64_t *nvalue) {
	...
	*/
	for (uint32_t d_start = 0; d_start < 4; d_start++) {
		was = 0;
		for (current_docid = const_cast<uint32_t *>(raw_integers) + d_start,
		    current_delta = deltas_buffer + d_start;
		     current_docid < raw_integers + source_integers;
		     current_docid += 4, current_delta += 4) {
			is = *current_docid;
...
int main(void) {
	...
	printf("void ANT_compress_qmx_d4::decodeArray(const uint32_t *source, uint64_t len, uint32_t "
	       "*to, uint64_t destination_integers)\n");
	printf("{\n");
	printf("__m128i byte_stream, byte_stream_2, tmp, tmp2, mask_21, mask_12, mask_10, mask_9, "
	       "mask_7, mask_6, mask_5, mask_4, mask_3, mask_2, mask_1, deltas;\n");
	...
	printf("deltas = _mm_castps_si128(_mm_xor_ps(_mm_cvtepu8_epi32(deltas), "
	       "_mm_cvtepu8_epi32(deltas)));\n");
	printf("\n");
	...
	for (instance = 0; instance <= 0xFF; instance++) {
		...
		if ((instance >> 4) == 0) {
			...
			*/
			...
			printf("\t\t\t_mm_store_si128((__m128i *)to + 10, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 11, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 12, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 13, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 14, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 15, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 16, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 17, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 18, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 19, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 20, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 21, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 22, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 23, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 24, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 25, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 26, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 27, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 28, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 29, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 30, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 31, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 32, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 33, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 34, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 35, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 36, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 37, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 38, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 39, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 40, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 41, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 42, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 43, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 44, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 45, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 46, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 47, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 48, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 49, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 50, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 51, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 52, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 53, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 54, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 55, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 56, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 57, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 58, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 59, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 60, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 61, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 62, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 63, deltas = _mm_add_epi32(deltas, "
			       "tmp));\n");
			...
			*/
			...
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 1, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 2, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 3, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 4, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 5, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 6, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 7, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 8, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 9, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 10, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 11, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 12, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 13, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 14, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 15, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 16, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 17, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 18, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 19, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 20, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 21, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 22, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 23, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 24, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 25, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 26, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 27, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 28, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 29, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 30, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			printf("\t\t\tbyte_stream = _mm_srli_epi64(byte_stream, 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 31, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_1)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_2)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_3)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_4)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_5)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_6)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_7)));\n");
			...
			printf("\t\t\t_mm_store_si128((__m128i *)to + 4, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, "
			       "4), _mm_srli_epi32(byte_stream, 7)), mask_7)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_9)));\n");
			...
			printf("\t\t\t_mm_store_si128((__m128i *)to + 3, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, "
			       "5), _mm_srli_epi32(byte_stream, 9)), mask_9)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_10)));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_12)));\n");
			...
			printf("\t\t\t_mm_store_si128((__m128i *)to + 2, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, "
			       "8), _mm_srli_epi32(byte_stream, 12)), mask_12)));\n");
			...
			*/
			...
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_cvtepu16_epi32(tmp)));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 1, deltas = "
			       "_mm_add_epi32(deltas,_mm_cvtepu16_epi32(_mm_castps_si128(_mm_movehl_ps(_mm_"
			       "castsi128_ps(tmp), _mm_castsi128_ps(tmp))))));\n");
			...
			*/
			printf("\t\t\tbyte_stream = _mm_load_si128((__m128i *)in);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(byte_stream, mask_21)));\n");
			printf("\t\t\tbyte_stream_2 = _mm_load_si128((__m128i *)in + 1);\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 1, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, "
			       "11), _mm_srli_epi32(byte_stream, 21)), mask_21)));\n");
			printf("\t\t\t_mm_store_si128((__m128i *)to + 2, deltas = "
			       "_mm_add_epi32(deltas,_mm_and_si128(_mm_srli_epi32(byte_stream_2, 11), "
			       "mask_21)));\n");
...
static uint32_t sequence[] = {
    13,   14,   15,   41,   59,   62,   63,   72,   76,   84,   89,   108,  115,  141,  142,  147,
    154,  157,  169,  174,  213,  229,  232,  237,  256,  264,  282,  283,  284,  285,  287,  292,
    301,  304,  325,  327,  333,  370,  373,  378,  383,  401,  404,  435,  438,  460,  465,  482,
    488,  500,  506,  508,  513,  523,  526,  538,  589,  603,  610,  618,  619,  621,  624,  651,
    670,  671,  681,  689,  691,  698,  700,  709,  725,  731,  737,  742,  748,  752,  770,  791,
    804,  806,  807,  818,  821,  843,  845,  861,  874,  935,  956,  968,  1019, 1029, 1035, 1066,
    1080, 1145, 1160, 1242, 1247, 1251, 1269, 1272, 1273, 1274, 1278, 1312, 1317, 1326, 1330, 1337,
    1338, 1363, 1380, 1432, 1492, 1500, 1508, 1512, 1534, 1541, 1590, 1616, 1618, 1690, 1719, 1752,
    1758, 1769, 1772, 1780, 1781, 1804, 1841, 1842, 1845, 1846, 1847, 1848, 1851, 1871, 1877, 1878,
    1880, 1881, 1882, 1883, 1897, 1899, 1903, 1904, 1910, 1914, 1918, 1921, 1922, 1923, 1925, 1927,
    1928, 1937, 1966, 1967, 1977, 1988, 1992, 2002, 2033};
...
void ANT_compress_qmx_d4::decodeArray(const uint32_t *source, uint64_t len, uint32_t *to,
                                      uint64_t destination_integers) {
	__m128i byte_stream, byte_stream_2, tmp, tmp2, mask_21, mask_12, mask_10, mask_9, mask_7,
	    mask_6, mask_5, mask_4, mask_3, mask_2, mask_1, deltas;
	...
	while (to < end) {
		...
		case 0x10:
			...
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			...
			_mm_store_si128((__m128i *)to + 4,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 5,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 6,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 7,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 8,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 9,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 10,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 11,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 12,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 13,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 14,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 15,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 16,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 17,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 18,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 19,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 20,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 21,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 22,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 23,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 24,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 25,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 26,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 27,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 28,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 29,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 30,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			byte_stream = _mm_srli_epi64(byte_stream, 1);
			_mm_store_si128((__m128i *)to + 31,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_1)));
			in += 16;
			to += 128;
		case 0x11:
		...
		case 0x20:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_2)));
		...
		case 0x30:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_3)));
		...
		case 0x40:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_4)));
		...
		case 0x50:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_5)));
		...
		case 0x60:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_6)));
		...
		case 0x70:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_7)));
			...
			_mm_store_si128((__m128i *)to + 4,
			                deltas = _mm_add_epi32(
			                    deltas, _mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, 4),
		...
		case 0x80:
			...
			_mm_store_si128((__m128i *)to + 1,
			                deltas = _mm_add_epi32(deltas, _mm_cvtepu8_epi32(tmp2)));
			tmp = _mm_castps_si128(_mm_movehl_ps(_mm_castsi128_ps(tmp), _mm_castsi128_ps(tmp)));
			_mm_store_si128((__m128i *)to + 2,
			                deltas = _mm_add_epi32(deltas, _mm_cvtepu8_epi32(tmp)));
			tmp2 = _mm_castps_si128(
			    _mm_shuffle_ps(_mm_castsi128_ps(tmp), _mm_castsi128_ps(tmp), 0x01));
			_mm_store_si128((__m128i *)to + 3,
			                deltas = _mm_add_epi32(deltas, _mm_cvtepu8_epi32(tmp2)));
		...
		case 0x90:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_9)));
			...
			_mm_store_si128((__m128i *)to + 3,
			                deltas = _mm_add_epi32(
			                    deltas, _mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, 5),
		...
		case 0xa0:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_10)));
		...
		case 0xb0:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_12)));
			...
			_mm_store_si128((__m128i *)to + 2,
			                deltas = _mm_add_epi32(
			                    deltas, _mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, 8),
		...
		case 0xc0:
			...
			_mm_store_si128((__m128i *)to + 1,
			                deltas = _mm_add_epi32(
			                    deltas, _mm_cvtepu16_epi32(_mm_castps_si128(_mm_movehl_ps(
		...
		case 0xd0:
			byte_stream = _mm_load_si128((__m128i *)in);
			_mm_store_si128((__m128i *)to,
			                deltas = _mm_add_epi32(deltas, _mm_and_si128(byte_stream, mask_21)));
			byte_stream_2 = _mm_load_si128((__m128i *)in + 1);
			_mm_store_si128(
			    (__m128i *)to + 1,
			    deltas = _mm_add_epi32(deltas,
			                           _mm_and_si128(_mm_or_si128(_mm_slli_epi32(byte_stream_2, 11),
			                                                      _mm_srli_epi32(byte_stream, 21)),
			                                         mask_21)));
			_mm_store_si128((__m128i *)to + 2,
			                deltas = _mm_add_epi32(
			                    deltas, _mm_and_si128(_mm_srli_epi32(byte_stream_2, 11), mask_21)));
			in += 32;
			to += 12;
		case 0xd1:
			byte_stream = _mm_load_si128((__m128i *)in);
